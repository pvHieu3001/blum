    ...
    async main() {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                const registerResult = await this.register(initData, { proxyIndex: i });
                if (registerResult.success) {
                    this.log('Đăng nhập thành công!', 'success');
                    const token = registerResult.token;
                    
                    const streakResult = await this.getStreak(token, { proxyIndex: i });
                    if (streakResult.success) {
                        this.log(`Đã điểm danh ${streakResult.data.streakDaysCount} ngày!`, 'info');
                    } else {
                        this.log(`Hôm nay bạn đã điểm danh rồi!`, 'info');
                    }
                    await this.visitUser(token, { proxyIndex: i });

                    const userInfoResult = await this.getUserInfo(token, { proxyIndex: i });
                    if (userInfoResult.success) {
                        ...
                        if (userInfoResult.tickets > 0) {
                            ...
                            await this.handleGamePlay(token, { proxyIndex: i });
                        }
                    } else {
                        this.log(`Không thể lấy thông tin người dùng: ${userInfoResult.error}`, 'error');
                    }

                    const availableTasksResult = await this.getAvailableTasks(token, { proxyIndex: i });
                    if (availableTasksResult.success) {
                        ...
                        for (const task of availableTasksResult.tasks) {
                            const taskResult = await this.handleTask(token, task, { proxyIndex: i });
