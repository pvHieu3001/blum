...
class MhayaAPIClient {
    ...
    async login(token, nickName) {
        ...
        return this.makeRequest('post', url, payload, token);
    }
    ...
    async main() {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                console.log(`========== Tài khoản ${i + 1} | ${nickName.green} | ip: ${proxyIP} ==========`);
                ...
                const tonAmount = item['4'].num;
                if (usdtAmount > 1 || tonAmount > 0) {
                    await this.writeUsdtToFile(i + 1, nickName, usdtAmount, tonAmount);
                }
